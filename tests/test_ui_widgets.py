...
def test_control_panel_buttons_logging(caplog):
    ...
    with patch("PyQt5.QtWidgets.QMessageBox.information"), \
        ...
        assert any("User action: Start Acquisition clicked" in r.message for r in caplog.records)
        caplog.clear()
        ...
        assert any("User action: Stop Acquisition clicked" in r.message for r in caplog.records)
        caplog.clear()
        ...
        assert any("User action: Baseline correction toggled" in r.message for r in caplog.records)
        caplog.clear()
        # 7. Export JSON
        panel.export_btn.click()
        assert any("User action: Export requested" in r.message for r in caplog.records)
        caplog.clear()

        # 8. Save Chart
        panel.save_chart_btn.click()
        assert any("User action: Save Chart requested" in r.message for r in caplog.records)
        caplog.clear()


        # 10. Shutdown
        panel.shutdown_btn.click()
        assert any("User action: Shutdown requested" in r.message for r in caplog.records)
        caplog.clear()
