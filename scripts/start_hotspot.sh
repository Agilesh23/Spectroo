...
SSID=$("$PYTHON_BIN" -c "import tomllib; c=tomllib.load(open('$CONFIG','rb')); print(c['hotspot']['ssid'])")
PASSWORD=$("$PYTHON_BIN" -c "import tomllib; c=tomllib.load(open('$CONFIG','rb')); print(c['hotspot']['password'])")
INTERFACE=$("$PYTHON_BIN" -c "import tomllib; c=tomllib.load(open('$CONFIG','rb')); print(c['hotspot']['interface'])")
GATEWAY_IP=$("$PYTHON_BIN" -c "import tomllib; c=tomllib.load(open('$CONFIG','rb')); print(c.get('hotspot', {}).get('gateway_ip', '10.42.0.1'))")
MDNS_HOSTNAME=$("$PYTHON_BIN" -c "import tomllib; c=tomllib.load(open('$CONFIG','rb')); print(c.get('hotspot', {}).get('mdns_hostname', 'spectroo.local'))")
...
if $SUDO ss -tlnp 2>/dev/null | grep -q ":53.*dnsmasq"; then
